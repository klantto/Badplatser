...
(async () => {
  ...
  const bathingsRes = await fetch(`https://gw-test.havochvatten.se/external-public/bathing-waters/v2/bathing-waters`);
  ...
  for (const bw of vgrBadplatser) {
    ...
    const measurementsRes = await fetch(`https://gw-test.havochvatten.se/external-public/bathing-waters/v2/bathing-waters/${bw.id}/measurements`);
